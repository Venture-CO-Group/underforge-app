...
interface WeeklyProgressButtonProps {
...
export default function WeeklyProgressButton({
  onPress,
  label,
  iconKey = 'bar_chart',
  containerStyle,
  accessibilityLabel,
}: WeeklyProgressButtonProps) {
  const { t } = useTranslation(['progress']);
  const resolvedLabel = label ?? t('progress:weeklyProgressButtonLabel');
  const resolvedA11y = accessibilityLabel ?? t('progress:weeklyProgressButtonA11y');
  return (
    <TouchableOpacity
      onPress={onPress}
